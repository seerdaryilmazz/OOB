package ekol.agreement.config.listener;
...
import ekol.agreement.domain.model.AuditedBaseEntity;
import ekol.hibernate5.domain.embeddable.UtcDateTime;
import ekol.resource.oauth2.SessionOwner;

@Component
public class AgreementDeleteEventListener implements DeleteEventListener {
...
//			new OnUpdateVisitor( source, id, entity ).process( entity, persister );
	...
	protected final void deleteEntity(
		...
		if(entity instanceof AuditedBaseEntity) {
			UtcDateTime s = new UtcDateTime(ZonedDateTime.now(ZoneId.of("UTC")).toLocalDateTime());
			AuditedBaseEntity e = AuditedBaseEntity.class.cast(entity);
			e.setDeleted(true);
			e.setDeletedAt(s);
			e.setLastUpdatedBy(sessionOwner.getCurrentUser().getUsername());
			e.setLastUpdated(s);

			Object[] state = persister.getPropertyValues(e);
			int[] dirtyProperties = persister.findDirty( state, currentState, entity, session );
			Object nextVersion = getNextVersion(session, entityEntry, persister);
...
//		TypeFactory.deepCopy( currentState, propTypes, persister.getPropertyUpdateability(), deletedState, session );
	...
	private Object getNextVersion(final EventSource session, final EntityEntry entry, final EntityPersister persister) throws HibernateException {

		if ( persister.isVersioned() ) {

			Object[] values = entry.getLoadedState();

			if ( entry.isBeingReplicated() ) {
				return Versioning.getVersion( values, persister );
			}
			else {
				final boolean isVersionIncrementRequired = isVersionIncrementRequired(persister);

				final Object nextVersion = isVersionIncrementRequired ?
						Versioning.increment( entry.getVersion(), persister.getVersionType(), session ) :
	...
	private boolean isVersionIncrementRequired( EntityPersister persister) {
		return Versioning.isVersionIncrementRequired(
